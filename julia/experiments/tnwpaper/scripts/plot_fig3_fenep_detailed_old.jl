...
function plot_row!(
    ...
    panel3_kind::Symbol=:M,   # :M or :eta_p
)
    ...
    Y3_sty = panel3_kind == :M ? M_sty :
             panel3_kind == :eta_p ? eta_sty :
             error("Unsupported panel3_kind = $panel3_kind")
    ...
    for row in rows
        Y3 = panel3_kind == :M ? row.M : row.eta_p
        Y3_e = panel3_kind == :M ? row.M_e : row.eta_p_e

        lines!(axM, row.Wi, Y3; color=row.col, linewidth=sty.line_lw)
        scatter!(axM, [row.Wi_e], [Y3_e];
            color=row.col,
            markersize=sty.marker_size,
            strokecolor=darken(row.col, 0.15),
            strokewidth=0.6
    ...
    ymin3, ymax3 = positive_bounds(Y3_sty,
        [panel3_kind == :M ? r.M : r.eta_p for r in rows]...;
        pad_low=0.08, pad_high=0.08
    )

    lines!(axM, [2 / 3, 2 / 3], [ymin3, ymax3];
        color=sty.ref_dark,
        linestyle=:dash,
        linewidth=0.95 * sty.line_lw / 2
    )
    ...
    # panel iii
    xmin3_x, xmax3_x = positive_bounds(Wi_sty, [r.Wi for r in rows]...; pad_low=0.08, pad_high=0.08)
    apply_log10_x!(axM, 0.1, xmax3_x)
    apply_log10_y!(axM, ymin3, ymax3)
    ...
    reltext!(axM, labels.steady[1], labels.steady[2], "steady"; sty=sty, color=sty.ref_dark)
...
function main(args)
    ...
    # --------------------------------------------------------
    # Panels i and ii together ~ one column width;
    # panel iii ~ one column width.
    # These are cell sizes, not including outer figure padding.
    w_small_cm = 3.0
    w_big_cm = 6.0
    row_h_cm = 6.0
    ...
    # Extra padding around the whole figure so protrusions
    # (axis labels, tick labels, panel letters, etc.) do not get clipped.
    fig_pad_pt = (1, 1, 1, 1)

    total_w_cm = 2w_small_cm + w_big_cm + 2col_gap_cm
    total_h_cm = 2row_h_cm + row_gap_cm
    ...
    axA3 = Axis(fig[1, 3], xlabel=L"\mathrm{Wi}", ylabel=L"M")

    axB1 = Axis(fig[2, 1], xlabel=L"t", ylabel=L"R")
    axB2 = Axis(fig[2, 2], xlabel=L"\varepsilon", ylabel=L"\mathrm{Wi}")
    axB3 = Axis(fig[2, 3], xlabel=L"\mathrm{Wi}", ylabel=L"\bar{\eta}_{\mathrm{p}}")
    ...
    # --------------------------------------------------------
    # manual label positions - tweak these only
    # --------------------------------------------------------
    LAB_A = (
        tag_i=(0.75, 0.85, "A (i)"),
        tag_ii=(0.8, 0.85, "(ii)"),
        tag_iii=(0.8, 0.85, "(iii)"), de_head_R=(0.7, 0.7),
        de_head_Wi=(0.1, 0.77),
        de_head_M=(0.35, 0.65), de_R=Dict(
            10.0 => (0.9, 0.7),
            3.0 => (0.80, 0.48),
            1.0 => (0.75, 0.4),
            0.1 => (0.75, 0.14),
            ...
            1.0 => (0.57, 0.6),
            0.1 => (0.45, 0.65),
        ), twothirds_Wi=(0.8, 0.47),
        twothirds_M=(0.29, 0.9),
        steady=(0.09, 0.4), param_lines=[
    ...
    LAB_B = (
        tag_i=(0.75, 0.88, "B (i)"),
        tag_ii=(0.8, 0.88, "(ii)"),
        tag_iii=(0.8, 0.88, "(iii)"), de_head_R=(0.6, 0.79),
        de_head_Wi=(0.2, 0.84),
        de_head_M=(0.05, 0.32), de_R=Dict(
            100.0 => (0.81, 0.79),
            30.0 => (0.8, 0.6),
            10.0 => (0.5, 0.5),
            1.0 => (0.18, 0.4),
        ...
        twothirds_M=(0.29, 0.9),
        steady=(0.1, 0.6), param_lines=[
            ("FENE-P", 0.6, 0.18, 1.10),
